...
class Missions extends StatefulWidget {
  ...
  DateTime date;
  //Missions.byDate({super.key, required this.date});
  @override
...
class _MissionsState extends State<Missions> {
  ...
  // @override
  // void dispose() {
  //   // מוודא שהרשימה היומית תמיד ריקה לפינה העדכון מרשימת המשימות הכוללת מהשרת
  //   setState(() {
  //     todayMissions = [];
  //   });
  //   super.dispose();
...
//--------------------------------------------------
  ...
  //   final url = Uri.https(
  //       'chick-chack-97c32-default-rtdb.firebaseio.com', 'mission-list.json');
  //   final response = await http.get(url);
  //   print(response.body);
...
//---------------------------------------------------
  ...
  Widget build(BuildContext context) {
    ...
    //DateTime currentDate = DateTime(widget.date.year, widget.date.month, widget.date.day);
    String dateLine =
        '${widget.date.day} / ${widget.date.month} / ${widget.date.year}';
    Widget mainContent;
    todayMissions = sortMissionsByDay(loadedMissions, widget.date);
    // List<Mission> todayMissions =
    //     sortMissionsByDay(loadedMissions, widget.date);
                        ...
                        children: [
                          ...
                          IconButton(
                              onPressed: () {
                                setState(() {
                                  todayMissions =
                                      []; // מנקה את המשימות הימיומיות בידיעה שיבקש מהן שוב
